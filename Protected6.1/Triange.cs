...
namespace Protected6._1
{
    class Triange : Figure
    {
        ...
        {
            Console.WriteLine($"Here is the perimeter of triangle: {xLength + yLength + zLength:f3}");
        }

        public override void CalculateSquare()
        {
            double p = (xLength + yLength + zLength) / 2;
            Console.WriteLine($"Here is the square of triangle: {Math.Sqrt(p*(p-xLength)*(p-yLength)*(p-yLength)):f3}");
        }
    }
}
